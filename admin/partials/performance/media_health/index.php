...
if (!class_exists('Npcink_Performance_Media_Health')) {
    class Npcink_Performance_Media_Health {
        ...
        public static function run($config) {
            ...
            add_action('wp_ajax_mabox_media_check', array(__CLASS__, 'ajax_check'));
            add_action('wp_ajax_mabox_media_fix_alt', array(__CLASS__, 'ajax_fix_alt'));
        }
        public static function ajax_check() {
            ...
            $large_images = $wpdb->get_results("SELECT ID, guid FROM {$wpdb->posts} WHERE post_type = 'attachment' AND post_mime_type LIKE 'image/%'");
            $large_count = 0;
            ...
            $chinese_names = $wpdb->get_results("SELECT ID, post_name FROM {$wpdb->posts} WHERE post_type = 'attachment'");
            $chinese_count = 0;
            ...
            $unused = $wpdb->get_results("SELECT p.ID FROM {$wpdb->posts} p WHERE p.post_type = 'attachment' AND NOT EXISTS (SELECT 1 FROM {$wpdb->posts} parent WHERE parent.post_content LIKE CONCAT('%', p.guid, '%') OR parent.post_excerpt LIKE CONCAT('%', p.guid, '%')) LIMIT 100");
            if (count($unused) > 0) {
