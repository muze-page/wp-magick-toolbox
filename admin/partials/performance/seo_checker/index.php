<?php
if (!class_exists('Npcink_Performance_Seo_Checker')) {
    class Npcink_Performance_Seo_Checker {
        private static $config;
        public static function run($config) {
            self::$config = $config;
            if (empty($config['enabled'])) return;
            add_action('wp_ajax_mabox_seo_check', array(__CLASS__, 'ajax_check'));
            add_action('wp_ajax_mabox_seo_fix_alt', array(__CLASS__, 'ajax_fix_alt'));
        }
