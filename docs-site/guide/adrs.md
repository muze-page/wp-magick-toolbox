...
> **后续状态（2026-07）**：Pre-GA 清理保留 React、TypeScript 与 Vite，但将三套依赖和工具链收口为 `vite/` 下的一套前端工程。`admin`、`count` 继续生成独立按页加载产物；没有 PHP enqueue 或 DOM 挂载消费者的 `vite/public` 已删除。当前决策与门禁见仓库 `docs/adr-pre-ga-admin-reset-2026-07-15.md`。
