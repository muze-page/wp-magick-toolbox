...
Npcink Site Toolbox 是一款面向中国 WordPress 站长的免费工具箱插件，涵盖站点优化、SEO、安全防护、媒体维护和国内生态对接等常用场景。

## 为什么选择 Npcink Site Toolbox？

### 一个插件，解决 80% 日常需求

WordPress 生态中有大量单一功能插件。Npcink Site Toolbox 将常用能力整合在同一套按需加载架构中，减少重复设置入口和维护成本。
...
- **前端**：React + TypeScript + Vite；管理外壳使用 WordPress 风格，复杂表单按需使用 Ant Design
- **图表**：ECharts + 自研统计组件
