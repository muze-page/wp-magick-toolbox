...
基于 GPL-2.0 开源协议发布，全部功能免费开放，不做功能分层，不做付费墙。通过技术支持、定制开发等增值服务盈利。
...
| 一键配置 | 7 套内置方案（个人博客、企业官网、B2 主题等），一键应用 |
| AI 审核 | DeepSeek / 阿里云 / 自定义 API，无 API 时自动降级到本地规则 |
...
- **图表**：ECharts + 自研统计组件
- **通信**：WordPress REST API（主）+ AJAX（兼容层）
